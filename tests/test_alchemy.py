...
def _cpu_slug():
    """BLAS kernels are dispatched per CPU model, so bitwise references are
    only meaningful on the machine class that produced them."""
    import re
    try:
...
def test_lbfgs_matches_stored_reference(small_config, synth_data, tmp_path):
    """Golden-file guard for fit_lbfgs_from_gram: every iterate must stay
    bitwise identical to the run that produced
    tests/data/lbfgs_reference_<cpu>.npz on this CPU model. The first
    reference (AMD EPYC, banff) was made by the version whose tracker held only
    time and total_loss, and the data_loss addition reproduced it bitwise.

    The synthetic gram is reproducible across processes (its block hashes are
    checked below), but the fixture's initialization is not, so the reference
    stores the init it used and later runs start from that same init. BLAS
    threads are pinned so the reduction order does not depend on the core
    count; the CPU model still selects the BLAS kernels, hence one reference
    per CPU model. On a machine without one, the first run writes it and
    skips. Regenerate only with a deliberate, documented change in the
    optimizer."""
    import os, hashlib
    from threadpoolctl import threadpool_limits
    ref_path = os.path.join(os.path.dirname(__file__), "data",
                            f"lbfgs_reference_{_cpu_slug()}.npz")
    have_ref = os.path.exists(ref_path)
    ref = np.load(ref_path) if have_ref else None
    model, regs, init, cwd = lbfgs_setup(small_config, synth_data,
                                         tmp_path / "ref")
    try:
        blocks = sorted(f for f in os.listdir("gram") if f.endswith(".npy"))
        hashes = np.array([hashlib.md5(np.ascontiguousarray(
            np.load(os.path.join("gram", f))).tobytes()).hexdigest()
            for f in blocks])
        if have_ref:
            assert list(ref["block_names"]) == blocks
            assert list(ref["block_hashes"]) == list(hashes), \
                "synthetic gram changed; the reference no longer applies"
            init = {k: ref[f"init_{k}"] for k in ref["init_keys"]}
        else:
            init = {k: init[k] for k in init.files}
        ...
        os.chdir(cwd)
    if not have_ref:
        np.savez(ref_path, x=result.x, fun=result.fun, nit=result.nit,
                 nfev=result.nfev, total_loss=hist,
                 block_names=np.array(blocks), block_hashes=hashes,
                 init_keys=np.array(list(init)),
                 **{f"init_{k}": v for k, v in init.items()})
        pytest.skip("reference written; rerun to compare")
    assert result.nit == int(ref["nit"]) and result.nfev == int(ref["nfev"])
    np.testing.assert_array_equal(result.x, ref["x"])
    np.testing.assert_array_equal(hist, ref["total_loss"])
    assert result.fun == float(ref["fun"])


def test_lbfgs_tracker_data_loss(small_config, synth_data, tmp_path):
    """The tracker's data_loss is the objective without the penalty. At the
    initial point it must equal sum_t w_t^2 n_t RMSE_t^2 from the gram, and
    total_loss - data_loss must equal the exact penalty at the same point."""
    import os
    model, regs, init, cwd = lbfgs_setup(small_config, synth_data,
                                         tmp_path / "dl")
    try:
        model.fit_lbfgs_from_gram(
            "gram", metadata_file=synth_data["metadata"],
            coverage_file=synth_data["coverage"], init_params=init,
            C_regularizers=regs, max_iter=5)
        tr = np.load("lbfgs_tracker.npz")
        assert set(tr.files) == {"time", "total_loss", "data_loss"}
        assert len(tr["data_loss"]) == len(tr["total_loss"]) > 1
        assert np.all(tr["data_loss"] <= tr["total_loss"])
        meta = np.load(synth_data["metadata"])
        gmeta = np.load("gram/gram_meta.npz")
        at_init = make_model(small_config)
        at_init.initialize_parameters(init)
        c = at_init.gram_coefficients()
        data = sum(float(meta[f"w_{t}"])**2 * float(gmeta[f"n_{t}"])
                   * alchemy.gram_rmse("gram", c, t=t)**2
                   for t in "ef" if float(meta[f"w_{t}"]) != 0)
        np.testing.assert_allclose(tr["data_loss"][0], data, rtol=1e-10)
        pen = np.sum((regs[1] @ init["coeff_1b"])**2)
        for k in (2, 3):
            pen += np.sum((regs[k] @ init[f"coeff_{k}b"]
                           @ init[f"pseudo_weights_{k}b"].T)**2)
        np.testing.assert_allclose(tr["total_loss"][0] - tr["data_loss"][0],
                                   pen, rtol=1e-10)
    finally:
        os.chdir(cwd)
